...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try { 	
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      if (!img) throw `💝Send gif or image*${usedPrefix + command}*`
      let out
      try {
    ...
    else throw 'The conversion has failed, try to send *image/video/gif* first and then respond with the command'
  }
}
